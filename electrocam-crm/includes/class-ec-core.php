...
class EC_Core {
	...
	public function render_appointment_meta_box( $post ) {
		...
		<?php $reminder_failure_attrs = $this->get_last_reminder_failure_status_markup_attributes( $post->ID ); ?>
		<p><strong><?php esc_html_e( 'Último fallo de recordatorio', 'electrocam-crm' ); ?>:</strong> <span class="<?php echo esc_attr( $reminder_failure_attrs['class'] ); ?>" data-reminder-failure-state="<?php echo esc_attr( $reminder_failure_attrs['state'] ); ?>"><?php echo esc_html( $this->get_last_reminder_failure_display_text( $post->ID ) ); ?></span></p>
		<hr>
	...
	public function render_client_orders_shortcode( $atts = array() ) {
		...
		if ( $page > 1 ) {
			...
			$previous_args['eco_client_page'] = $page - 1;
		...
		if ( $has_more ) {
			...
			$next_args['eco_client_page'] = $page + 1;
	...
	public function render_client_quotations_shortcode( $atts = array() ) {
		...
		$has_more = count( $quotations ) > $limit;
		if ( $has_more ) {
			$quotations = array_slice( $quotations, 0, $limit );
		}
	...
	public function render_client_appointments_shortcode( $atts = array() ) {
		...
		if ( empty( $appointments ) ) {
			if ( $filter_status || ! $show_past || $date_from || $date_to || $range ) {
	...
	public function render_operator_orders_shortcode( $atts = array() ) {
		...
		if ( $page > 1 ) {
			$previous_args = $pagination_base_args;
			$previous_args['eco_operator_page'] = $page - 1;
			$pagination_links[] = '<a href="' . esc_url( add_query_arg( $previous_args ) ) . '">' . esc_html__( 'Anterior', 'electrocam-crm' ) . '</a>';
		}
		if ( $has_more ) {
			$next_args = $pagination_base_args;
			$next_args['eco_operator_page'] = $page + 1;
			$pagination_links[] = '<a href="' . esc_url( add_query_arg( $next_args ) ) . '">' . esc_html__( 'Siguiente', 'electrocam-crm' ) . '</a>';
		}
		if ( ! empty( $pagination_links ) ) {
			$output .= '<p class="ec-pagination-links">' . implode( ' | ', $pagination_links ) . '</p>';
...
%2$s", 'electrocam-crm' ),
	 ...
	 */
	/**
	 * Obtiene estado semántico del último fallo de recordatorio.
	 *
	 * @param int $appointment_id ID de cita.
	 * @return string failed|clear
	 */
	private function get_last_reminder_failure_state( $appointment_id ) {
		$appointment_id = absint( $appointment_id );
		$last_reminder_failed_at = (string) get_post_meta( $appointment_id, 'ec_last_reminder_email_failed_at', true );
		$default_state = ! empty( $last_reminder_failed_at ) ? 'failed' : 'clear';
		$state = apply_filters( 'ec_reminder_failure_state', $default_state, $appointment_id );
		$state = is_string( $state ) ? sanitize_key( $state ) : '';

		if ( ! in_array( $state, array( 'failed', 'clear' ), true ) ) {
			$state = 'clear';
		}
	...
	/**
	 * Construye atributos semánticos de estado de fallo de recordatorio.
	 *
	 * @param int $appointment_id ID de cita.
	 * @return array{class:string,state:string}
	 */
	private function get_last_reminder_failure_status_markup_attributes( $appointment_id ) {
		$state = $this->get_last_reminder_failure_state( $appointment_id );

		return array(
			'class' => 'ec-reminder-failure-status ec-reminder-failure-status--' . $state,
			'state' => $state,
		);
	}
	...
	private function render_last_reminder_failure_html( $appointment_id ) {
		...
		$attributes = $this->get_last_reminder_failure_status_markup_attributes( $appointment_id );

		return '<small class="' . esc_attr( $attributes['class'] ) . '" data-reminder-failure-state="' . esc_attr( $attributes['state'] ) . '">' . esc_html__( 'Último fallo de recordatorio:', 'electrocam-crm' ) . ' ' . esc_html( $display ) . '</small>';
	}


	/**
	 * Formatea fecha UTC para visualización local + UTC.
	 *
	 * @param string $utc_datetime Fecha en UTC.
	 * @return string
	 */
	private function format_utc_datetime_for_display( $utc_datetime ) {
		...
		if ( empty( $utc_datetime ) || ! preg_match( '/^\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}$/', $utc_datetime ) ) {
			return __( 'No registrado', 'electrocam-crm' );
		}
