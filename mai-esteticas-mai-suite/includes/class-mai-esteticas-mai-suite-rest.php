...
class Mai_Esteticas_Mai_Suite_REST
{
    ...
    {
        global $wpdb;
        $rows = $wpdb->get_results("SELECT id, name, address, phone, timezone FROM {$wpdb->prefix}mai_sedes WHERE is_active = 1", ARRAY_A);
    ...
    public function get_services(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $sql = "SELECT id, sede_id, category, name, duration_minutes, price, payment_rule, consent_required FROM {$wpdb->prefix}mai_services WHERE is_active = 1";
        if ($sede_id > 0) {
